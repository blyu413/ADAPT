...
class _TaskCfg:
    ...
    env_cls: type | None  # Custom environment class
...
def register_mjlab_task(
    ...
    Args:
      task_id: Unique task identifier (e.g., "Ref-NoMoMo-Flat-Unitree-G1").
      ...
      env_cls: Optional custom environment class. If None, uses ManagerBasedRlEnv.
    """
...
def load_env_cfg(task_name: str, play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    """
    return deepcopy(_REGISTRY[task_name].env_cfg if not play else _REGISTRY[task_name].play_env_cfg)
